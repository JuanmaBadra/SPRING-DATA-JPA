...
import com.example.restfulApi.repository.LocalRepository;
...
public class LocalServiceImpl implements LocalService {
    ...
    public Local updateLocal(Long id, Local local) {
        Local locals= localRepository.findById(id).get();
        //valido
        if(Objects.nonNull(local.getCode()) && !"".equalsIgnoreCase(local.getCode())){
            locals.setCode(local.getCode());
        }
        if(Objects.nonNull(local.getName()) && !"".equalsIgnoreCase(local.getName())){
            locals.setName(local.getName());
        }


        return localRepository.save(locals);
    }
    ...
    @Override
    public Local findLocalById(Long id) throws LocalNotFound {
        Optional<Local> local= localRepository.findById(id);
        if(!local.isPresent()){
            throw new LocalNotFound("Local is not available");
        }
        return local.get();
    }
